...
const TARGET = {
  ...
  period: '2021H1',
  securityCode: '300866',
  searchKey: '安克创新2021年半年度报告',
};

async function discoverOfficialPdf() {
  const endpoint = 'https://www.cninfo.com.cn/new/hisAnnouncement/query';
  const form = new URLSearchParams({
    pageNum:'1', pageSize:'30', column:'szse', tabName:'fulltext', plate:'sz',
    stock:'', searchkey:TARGET.searchKey, secid:'', category:'', trade:'',
  ...
  assert.equal(candidates.length>=1,true,`No full 2021H1 report discovered. Candidate count=${rows.length}`);
...
const chain=runC04Chain(parser,{
  runId:'CHAIN-BLIND-02-S10-FIRST',
  priorAdjustedYoy:[], accountingAdjustmentRecurring:null,
