...
更新：2026-09-13。网页与模型接入已合并。[紧凑备忘录 V0.3](MEMO_COMPACT_V0.3.md)的 [第 10 次验收](MEMO_CONTENT_REVIEW_RUN_10.md)已完成：34 项普通 CI、三份真实技术流程通过，28 文件归档和全部 18 段初审完成；原样内容仍须修订。PR #16 已于 9 月 10 日合并；[人工修订留痕](MEMO_HUMAN_REVISION_V0.1.md)随后获批并在分支实现，专业关卡仍待复核。
...
产品入口：材料更新并保存 → 版本历史 → 选择该版本 → AI 研究备忘录 → 生成 → 核对引用 → 原稿接受/退回或建立人工修订稿 → 独立审核 → 导出 Markdown 和 JSON 审计记录。

新增[人工修订 V0.1](MEMO_HUMAN_REVISION_V0.1.md)已获批并在分支实现：正文和引用另存版本，保存默认待复核，再次修订不继承旧稿接受。原始模型调用、审核和研究快照保持；此轮实现、测试、提交和使用说明一并交付，不增加真实模型调用。
...
28 文件归档哈希、原始 JSON、请求与快照重建及 52 项离线一致性核对完成。已逐段审阅全部 18 段：第一份反证存在漏引和含混句；第二、三份可供人工使用，但引用及期间表述尚未全部符合约定。详细处置见 [完整审阅](MEMO_CONTENT_REVIEW_RUN_10.md)。技术成功保留，原始自动接受事件仍仅作交互证据，不能当作用户或专业人员已接受。第 10 次结束后提出的人工修订和独立版本审核已于 2026-09-13 获明确批准，本分支交付该功能；新增真实模型调用预算仍为零。
