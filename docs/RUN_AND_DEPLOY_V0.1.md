...
2026-09-20 更新：#24 / #25 已合并。新手动问题验收见 [固定六请求批次](QUESTION_LIVE_ACCEPTANCE_V0.1.md)。两个生成路由均声明 180 秒处理窗口；提供方仍为 150 秒。`npm run build` 后写入本地构建来源，`npm run release:preflight` 检查构建与当前干净提交一致，输出不含密钥的记录，全程零模型请求。存在未提交改动、缺构建来源或提交不符会拒绝标记为发布就绪。

此预检不替代目标平台预演，也不证明远端实际运行版本。上线时记录同一 SHA、构建 ID、依赖锁摘要、origin 与平台时限；以 GET 检查配置，按已批准计划另行验证真实生成。回滚：保留浏览器导出及当前部署记录，切回上一份已验收提交重新构建/启动，不删除研究台账；恢复旧程序不会回滚用户研究数据。平台尚未选定，本轮未部署。
...
2. **时限必须核对。** 两个 API 路由声明均为 `maxDuration = 180`，提供方中止上限仍为 150 秒。平台、网关与代理须实际允许该窗口；路由声明不能突破平台自身限制。未改变输出 token 额度。[Next.js 路由配置](https://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config)
