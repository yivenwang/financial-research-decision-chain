...
核对日期：2026-09-14。基于 PR #22 `07271cd`；本轮准备可执行方案，未选择付费平台、创建线上服务或修改密钥。应用的 Node.js / Next.js 运行方式已在 PR #22 验收；部署平台的实际限制需在选定后核对。

## 三种用途

| 用途 | 运行入口 | 模型与数据行为 |
| --- | --- | --- |
| 只读历史回放 | [离线核对包](DEMO_REPLAY_V0.1.md)的 `index.html` | 展示已归档三份真实输出与待确认建议，不联网生成，不写浏览器版本库 |
...
`npm test` 是 `apps/web` 的定向测试入口；`npm run test:runtime` 校验生产 HTML、CSS、PDF worker。不要执行会读取本轮禁止材料的根目录全量历史测试。PR #22 已验证 42 项及构建，本轮新增的离线工具有独立测试。
...
| 项目 | 本轮状态 | 发布时需要的证据 |
| --- | --- | --- |
| 源码与运行版本 | PR #22 已有构建/浏览器证据；本轮收尾有独立 PR | 记录实际部署的 SHA |
...
| 正式内容审核 | 修订建议已准备，尚未由本人签署 | 真实修订者和审核者明确确认准确版本 |
